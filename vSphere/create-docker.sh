...
pushd "$export_dir"
  ...
  image_id=$(docker images --no-trunc "${imagename}" | awk "{if (\$2 == \"latest\") print \$3}")
  ...
  mkdir rootfs
  docker run --cidfile "$cidfile" $imagename /bin/bash
  docker export "$(cat "$cidfile")" | tar --exclude="dev/*" -xf - -C ./rootfs/
  docker stop "$(cat "$cidfile")"
popd

# compress all the things
tar -czf ${imagename}-image-1.0.tgz -C "$export_dir" .
